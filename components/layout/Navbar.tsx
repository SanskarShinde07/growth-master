  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-black/90 backdrop-blur-md border-b border-white/5 py-4" : "bg-transparent py-6"}`}>
      <div className="container-custom flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center group focus-ring rounded-lg px-2 -ml-2" aria-label="growth master home">
        ...
        <div className="hidden md:block">
          <a href="https://wa.me/918830614946" target="_blank" rel="noopener noreferrer" className="bg-[#25D366] text-black px-7 py-2.5 rounded-full text-sm font-bold shadow-[0_0_15px_rgba(37,211,102,0.3)] transition-all duration-300 hover:shadow-[0_0_25px_rgba(37,211,102,0.5)] hover:-translate-y-0.5 flex items-center justify-center gap-2 lowercase">
      ...
      <div className={`md:hidden absolute top-full left-0 w-full bg-black/95 backdrop-blur-xl border-b border-white/5 transition-all duration-300 overflow-hidden ${isMobileMenuOpen ? "max-h-[400px] py-6 opacity-100" : "max-h-0 py-0 opacity-0 pointer-events-none"}`}>
        <div className="container-custom flex flex-col gap-6">
          {navLinks.map((link) => (
            <Link key={link.name} href={link.href} className="text-xl font-medium text-[#A1A1AA] hover:text-white lowercase block" onClick={() => setIsMobileMenuOpen(false)}>
              {link.name}
            </Link>
          ))}
          {/* Mobile Menu CTA Button */}
          <a href="https://wa.me/918830614946" target="_blank" rel="noopener noreferrer" className="bg-[#25D366] text-black text-center px-6 py-4 rounded-full font-bold w-full mt-4 flex items-center justify-center gap-2 lowercase shadow-[0_0_15px_rgba(37,211,102,0.2)]" onClick={() => setIsMobileMenuOpen(false)}>
